...
export default function Home() {
  ...
  async function deleteFromReadList(id: string) {
    try {
      await axios
        .delete("/api/readlist", {
          data: { email: session?.user?.email, bookId: id },
        })
        .then((res) => {
  ...
  return (
    <div
      className={`min-h-screen w-screen flex flex-col items-center justify-start md:px-16 pt-10 duration-200 dark:text-white dark:bg-nifty-black text-black bg-white `}
    >
      ...
      <div className="w-full px-5">
        ...
        <OptionToggle
          options={["Bookmarks", "Readlist", "Minted"]}
          selectedOption={toggle}
          setOption={setToggle}
        />
        {readList.length > 0 && toggle == "Readlist" ? (
          <div>
          ...
          <>
            {toggle == "Readlist" && (
              <div className="w-full h-80 flex flex-col text-center items-center justify-center">
                <h3
                  className={`text-xl font-semibold duration-200 dark:text-white text-gray-500 mb-3`}
                >
                  Your shelf seems empty
                </h3>
                <h3 className="text-lg font-medium text-nifty-gray-1 mb-5 flex max-md:flex-col gap-2 items-center">
                  Add some books from{" "}
                  <button
                    onClick={() => {
                      router.push("/explore");
                    }}
                    className="h-10 w-32 bg-gray-200 font-semibold hover:-translate-y-1 duration-200 text-black rounded-lg"
                  >
                    Explore
                  </button>
                </h3>
        ...
        {slicedBookmark.length > 0 && toggle == "Bookmarks" ? (
          ...
          <>
            {toggle == "Bookmarks" && (
              <div className="w-full text-center h-80 flex flex-col items-center justify-center">
                <h3
                  className={`text-xl font-semibold duration-200 dark:text-white text-gray-500 mb-3`}
                >
                  Seems like you haven't bookmarked anything
                </h3>
                <h3 className="text-lg font-medium text-nifty-gray-1 mb-5 flex max-md:flex-col gap-2 items-center">
                  Find a book to read{" "}
                  <button
        ...
        {/* @ts-ignore */}
        {mintList?.length > 0 && toggle == "Minted" ? (
          <div>
            ...
            {toggle == "Minted" && (
              <div className="w-full text-center h-80 flex flex-col items-center justify-center">
                <h3
                  className={`text-xl font-semibold duration-200 dark:text-white text-gray-500 mb-3`}
                >
                  Seems like you haven't minted anything
                </h3>
                <h3 className="text-lg font-medium text-nifty-gray-1 mb-5 flex max-md:flex-col gap-2 items-center">
                  Find an author to support{" "}
                  <button
                    onClick={() => {
                      router.push("/explore");
                    }}
