...
export const GlobalContextProvider = ({
  ...
  async function ensImageFetcher() {
    try {
      ...
      if (user && address && session?.role != "ANONYMOUS") {
        ...
        if (ensAvatar) {
          await axios
            .patch("/api/user/" + user?.email, { profileImage: ensAvatar })
            .then((res) => {
              getUser();
          ...
          await axios
            .patch("/api/user/" + user?.email, {
              profileImage: response.image.pngUrl,
            })
            .then((res) => {
              getUser();
              window?.location.reload();
  ...
  async function ensNameFetcher() {
    try {
      ...
      if (user && address && session?.role != "ANONYMOUS") {
        ...
        if (ensName) {
          await axios
            .patch("/api/user/" + user?.email, { username: ensName })
            .then((res) => {
              window?.location.reload();
            });
        }
  ...
  const [walletNotRegistered, setWalletNotRegistered] =
  ...
  async function getAllBooks() {
    try {
      const trendingBooksResponse = await axios.get(
        "/api/book/?limit=10&type=trending",
      );
      const latestBooksResponse = await axios.get(
        "/api/book/?limit=10&type=latest",
      );
      const boostedBooksResponse = await axios.get(
        "/api/book/?limit=10&type=boosted",
      );
      console.log(
        "Fetched books: ",
        trendingBooksResponse.data.data.length,
        latestBooksResponse.data.data.length,
        boostedBooksResponse.data.data.length,
      );
      const publishedBooks = latestBooksResponse.data.data.filter(
        (item: any) =>
          item.isPublished && !item.isHidden && !item.isAdminRemoved,
      );
      const boostedBooks = boostedBooksResponse.data.data.filter(
        (item: any) =>
          item.isBoosted &&
          Number(item.isBoosted) > Date.now() &&
          !item.isAdminRemoved,
      );
  ...
  return (
    ...
    >
      {walletNotRegistered &&
        (pathname.split("/")[2] == "makeCollection" ||
